...
**A QR login cannot be resumed, and phone login can.** Telegram announces the scan on the connection that exported the code, and the authorization exists only once that same connection redeems it — there is no offline redemption. So `start_qr_login` deliberately writes nothing to disk: persisting a pre-authorization auth key for it would be a secret nothing can ever use. Phone login is different: `sendCode` settles the data centre before it returns, so the session persisted right after it is the one `complete_login` needs, which is why that flow survives a restart. If a QR change tempts you to add persistence, re-read this — two rounds of patching went into learning it.
