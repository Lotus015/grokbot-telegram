...
**The persisted login session must track the live auth key.** An authorization attaches to the auth key the client holds at that moment, and QR login replaces that key when it migrates data centre. If the pending file still holds the key from before the switch, a process killed after the scan resumes something nobody authorized — Telegram lists the new device while `isAuthorized()` says false. `refreshPendingSession` is called after a DC switch, when the scan update lands, and before handing back a fresh code. Keep those calls if you touch the QR flow.
