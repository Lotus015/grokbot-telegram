...
describe("publish config", () => {
  it("is an npx-publishable package with no secrets", () => {
    ...
    assert.equal(pkg.name, "cursor-telegram-user-mcp");
    assert.equal(pkg.license, "MIT");
    assert.equal(pkg.bin?.["cursor-telegram-user-mcp"], "dist/index.js");
    assert.equal(pkg.bin?.["telegram-user-login"], "dist/login.js");
